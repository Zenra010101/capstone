...
public static class DataSeeder
{
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        // Create default branches before users, products, and inventory.
        await EnsureDefaultBranchesAsync(context);
        ...
        var bootstrapDemoAccounts = configuration.GetValue(
            "Seed:BootstrapDemoAccounts",
            defaultBootstrap);
        ...
        var includeSampleProducts = configuration.GetValue(
            "Seed:IncludeSampleProducts",
            false);
    ...
    private static async Task EnsureDefaultBranchesAsync(AppDbContext context)
    {
        // Do not create duplicates if branches already exist.
    ...
    /// <summary>
    /// Ensures roles exist; demo users only when
    /// <paramref name="bootstrapDemoAccounts"/> is true.
    /// </summary>
    private static async Task EnsureDefaultAccountsAsync(
        AppDbContext context,
        bool bootstrapDemoAccounts)
    {
        ...
        var ownerRole = await context.Roles
            .FirstAsync(r => r.Name == RoleNames.Owner);

        var cashierRole = await context.Roles
            .FirstAsync(r => r.Name == RoleNames.Cashier);

        if (!await context.Users.AnyAsync(
            u => u.Email == "owner@gensanpos.com"))
        {
        ...
        if (!await context.Users.AnyAsync(
            u => u.Email == "cashier@gensanpos.com"))
        {
            context.Users.Add(new User
            {
    ...
    /// <summary>
    /// One-time style backfill for sale lines created before
    /// cost snapshots existed.
    /// </summary>
    private static async Task BackfillSaleSnapshotsAsync(
        AppDbContext context)
    {
        ...
        {
            sale.GrossProfit = sale.Items
                .Sum(i => i.ProfitAmount);
        }

        await context.SaveChangesAsync();
    }

    private static async Task BackfillGrsFieldsAsync(
        AppDbContext context)
    {
        ...
        foreach (var g in slips)
        {
            ...
            foreach (var item in g.Items.Where(
                i => i.SellingPriceAtSale == 0))
            {
                item.SellingPriceAtSale = item.UnitPrice;
            }
        }
